    ...
    authenticate(credentials = {}) {
        ...
        this.auth = new Promise((resolve, reject) => {
            ...
            let opts = {
                method: 'POST',
                json: true,
                uri: Object.assign({}, this.auth_url, { pathname: '/v3/auth/tokens' }),
                headers: {
            ...
            this._request(opts, (err, res, payload) => {
                ...
                let { token: {catalog: serviceCatalog, project, issued_at, expires_at} } = payload;
                const token = {
                    ...
                    if (type === 'object-store' && name === 'swift') {
                        for (let endpoint of endpoints) {
                            if (endpoint.region && this.region && endpoint.region.toLowerCase() === this.region.toLowerCase()) {
                                if (endpoint.interface === this.endpoint_type) {
                                    this.storage_url = Url.parse(endpoint.url);
                                    break services;
                                }
                ...
                if(!this.storage_url.pathname) {
                    const e = new Error(`Cannot find requested endpoint type ${this.endpoint_type}`);
                    e.endpoints = serviceCatalog
                      .find(x=>x.type === 'object-store' && x.name === 'swift')
                      .endpoints
                      .filter(x => x.region && this.region && x.region.toLowerCase() === this.region.toLowerCase());
                    return reject(e);
                }
                resolve(token);
            });
